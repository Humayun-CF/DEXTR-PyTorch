...
class ProstateSegmentation(data.Dataset):

    category_names = ['background',
                      'central_zone', 'peripheral_zone']

    def __init__(self,
                 root=Path.db_root_dir('prostate'),
                 split='val',
                 transform=None,
                 download=False,
                 preprocess=False,
        ...
        # self.root = '/data/Humayun/Arterys/Dataset_NCI_ISBI_Prostate_Challenge/Prostate_Arterys'
        self.root = root
        _prostate_root = os.path.join(self.root, '')
        _mask_dir = os.path.join(_prostate_root, 'SegmentationObject')
        _cat_dir = os.path.join(_prostate_root, 'SegmentationClass')
        _image_dir = os.path.join(_prostate_root, 'JPEGImages')
        ...
        self.obj_list_file = os.path.join(_prostate_root, 'ImageSets', 'Segmentation',
                                          '_'.join(self.split) + '_instances' + area_th_str + '.txt')

        # train/val/test splits are pre-cut
        _splits_dir = os.path.join(_prostate_root, 'ImageSets', 'Segmentation')
    ...
    def _make_img_gt_point_pair(self, index):
        ...
        # Find the boundary pixels of objects by selecting on boundary pixel value which is 255
        _void_pixels = (_tmp == 255)
        # Remove the boundary pixels by replacing the pixel values with 0
        _tmp[_void_pixels] = 0
    ...
    def __str__(self):
        return 'Prostate(split=' + str(self.split) + ',area_thres=' + str(self.area_thres) + ')'
